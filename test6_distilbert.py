...
# Load DistilBERT sentiment model (more stable on Streamlit Cloud)
...
def analyze_sentiment(text):
    ...
    return label_map.get(result['label'].upper(), "neutral"), result['score']
...
def build_sentiment_graph(articles, center_label="Company"):
    ...
    for idx, article in enumerate(articles):
        ...
        G.add_edge("center", node_id, color=color)
    ...
    output_file = f"sentiment_graph_{uuid.uuid4()}.html"
...
if st.button("Analyze News"):
    if not company_name.strip():
        ...
        if news_articles:
            ...
            relevant_articles = [a for a in news_articles if is_article_relevant(a, company_name)]
